 ...
 */
public class IdentCode extends HttpServlet {
    public static final int WIDTH = 120;
    public static final int HEIGHT = 30;

    @Override
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        //验证码是一张随机图片，java中要构建一张随机图片，用BufferedImage类，可以在内存中构建一张图片
        //输出图片可以用imageio
        ...
        //控制所有浏览器不要缓存，否则只能刷新页面，只是回车地址栏不能够获得新验证码
    ...
    private void drawRandomNum(Graphics2D g) {
        ...
        int x= 5;
        for(int i=0;i<4;i++){
            int degree = new Random().nextInt()%30;//旋转度数正负30度，nextInt(30)值会往一边转
            String ch =  base.charAt(new Random().nextInt(base.length()))+"";
            g.rotate(degree*Math.PI/180, x, 20); //设置旋转角度，这个函数接受的是弧度，用角度转换一下
            g.drawString(ch, x, 20);
            g.rotate(-degree*Math.PI/180, x, 20);//清掉本次的旋转角度，否则旋转弧度叠加
            x+=30;
        }
    }
